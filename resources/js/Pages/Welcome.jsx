  ...
  import { 
    ...
    ChevronRight, Trophy, Image as ImageIcon 
  } from 'lucide-react';
...
export default function Welcome({ sorteo, otrosSorteos = [], ganadores = [] }) {
    ...
    const winnersWrapRef = useRef(null);
    const winnersTrackRef = useRef(null);
    const winnersXRef = useRef(0);
    const winnersWidthRef = useRef(0);
    const isWinnersPausedRef = useRef(false);
    const [isWinnersPaused, setIsWinnersPaused] = useState(false);

  const heroBackgrounds = (sorteo?.imagen_hero_url || sorteo?.imagen_hero) ? [sorteo.imagen_hero_url || sorteo.imagen_hero] : [
    ...
    const getDrawDate = (item) => item?.fecha_sorteo || item?.fecha_fin || null;

    const setWinnersPaused = (value) => {
      isWinnersPausedRef.current = value;
      setIsWinnersPaused(value);
    };

    const grandWinners = ganadores;
    const singleWinner = grandWinners.length === 1;

    // Auto-movimiento del carrusel (sin duplicar): se reinicia desde la derecha al salir por completo.
    useEffect(() => {
      const wrap = winnersWrapRef.current;
      const track = winnersTrackRef.current;
      if (!wrap || !track) return;
      if (grandWinners.length <= 1) return;

      const updateSizes = () => {
        winnersWidthRef.current = track.scrollWidth;
      };
    ...
    useEffect(() => {
      ...
      const track = winnersTrackRef.current;
      if (!wrap || !track) return;
      if (grandWinners.length === 0) return;

      let rafId = 0;
      let lastTs = 0;
      const speed = 100; // px/seg

      // Inicia visible
      winnersXRef.current = 0;
      track.style.transform = `translateX(${winnersXRef.current}px)`;
      ...
      const step = (ts) => {
        ...
        if (!isWinnersPausedRef.current) {
          const trackWidth = winnersWidthRef.current || track.scrollWidth;
          const wrapWidth = wrap.clientWidth;

          winnersXRef.current -= (speed * dt) / 1000;
          if (winnersXRef.current <= -trackWidth) {
            winnersXRef.current = wrapWidth;
          }
          track.style.transform = `translateX(${winnersXRef.current}px)`;
        }
    ...
    return (
      <PublicLayout isLoggedIn={!!auth?.user} currentUser={auth?.user}>
            ...
            <div className="flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-20 max-w-6xl mx-auto">
                ...
                {sorteo && getDrawDate(sorteo) && (
                  <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/20 px-4 py-2 rounded-full shadow-lg mb-6">
                    ...
                    <p className="text-white font-bold uppercase tracking-widest text-xs md:text-sm">
                      {new Date(getDrawDate(sorteo)).toLocaleDateString('es-PE', { day: '2-digit', month: 'long', year: 'numeric' })}
                    </p>
                  </div>
                )}
              ...
              {sorteo && (
                <div className="w-full max-w-md shrink-0 relative mt-12 lg:mt-0">
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 md:p-8 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)] border border-white dark:border-slate-800 relative z-10 transform transition-transform hover:-translate-y-2 duration-300">
                    
                    <div className="absolute -top-5 right-6 bg-[#FF0000] text-white px-4 py-2 rounded-xl shadow-lg transform rotate-3 font-black text-lg border-2 border-white dark:border-slate-900">
                      ¡SOLO S/ {Math.floor(sorteo.precio_ticket)}!
                    </div>

                    <div className="text-center mb-6 pt-4">
                      <p className="text-emerald-700 dark:text-emerald-400 font-bold uppercase tracking-wider text-sm mb-2">Compra tu ticket digital</p>
                      <div className="flex items-start justify-center gap-1 text-slate-900 dark:text-slate-100">
                    ...
                    <div className="bg-emerald-50/50 dark:bg-emerald-950/30 rounded-2xl p-5 mb-6 border border-emerald-100 dark:border-emerald-900/50">
                      <p className="text-xs text-emerald-800 dark:text-emerald-300 font-bold mb-3 uppercase text-center tracking-wider">Métodos de pago oficiales</p>
                      <div className="flex flex-col items-center gap-4">
                        <div className="bg-white dark:bg-slate-800 p-2 rounded-2xl shadow-inner border border-emerald-100 dark:border-emerald-800">
                      ...
                      <div className="mt-4 pt-4 border-t border-emerald-100 dark:border-emerald-900/50 text-center">
                        <p className="text-[10px] text-emerald-700 dark:text-emerald-400 uppercase font-bold tracking-wider mb-1">A nombre de:</p>
                        <p className="text-sm font-black text-slate-800 dark:text-slate-200">AGROINVERSIONES PERUVIAN ECOLOGIC S.A.C</p>
                      </div>
                    </div>
                    
                    <a href="/participar" className="w-full bg-[#25D366] hover:bg-[#20B858] text-white font-black text-xl py-5 rounded-2xl shadow-[0_6px_0_#1DA851] active:shadow-[0_0px_0_#1DA851] active:translate-y-[6px] transition-all flex items-center justify-center gap-2 group">
        ...
        {otrosSorteos.length > 0 && (
          <section className="py-14 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 transition-colors duration-300">
            <div className="container mx-auto px-4 max-w-6xl">
              <div className="text-center mb-10">
                <h2 className="text-3xl font-black text-slate-900 dark:text-white uppercase">Más Sorteos Activos</h2>
                <div className="w-16 h-1.5 bg-emerald-500 mx-auto rounded-full mt-4"></div>
                <p className="text-slate-500 dark:text-slate-400 mt-4 font-medium">¡Tienes más oportunidades de ganar! Participa en varios sorteos a la vez.</p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {otrosSorteos.map((s) => {
                  ...
                  return (
                    <div key={s.id} className="bg-white dark:bg-slate-800 rounded-2xl border-2 border-slate-100 dark:border-slate-700 hover:border-emerald-300 dark:hover:border-emerald-500 shadow-sm hover:shadow-xl transition-all group overflow-hidden flex flex-col">
                      {/* Banner superior */}
                      ...
                      {(s.imagen_hero_url || s.imagen_hero) ? (
                        <img src={s.imagen_hero_url || s.imagen_hero} alt={s.nombre} className="w-full h-full object-cover opacity-60 group-hover:scale-105 transition-transform duration-500" />
                        ) : (
                      ...
                      <div className="p-5 flex flex-col flex-1">
                        <div className="flex items-center justify-between mb-4">
                          <div>
                            ...
                            <p className="text-2xl font-black text-emerald-700 dark:text-emerald-400">S/ {parseFloat(s.precio_ticket).toFixed(2)}</p>
                          </div>
                          {totalP > 0 && (
                            <div className="text-right">
                              <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Total premios</p>
                              <p className="text-2xl font-black text-slate-800 dark:text-slate-200">{totalP}</p>
                            </div>
                          )}
                        </div>
                        ...
                        {getDrawDate(s) && (
                          <p className="text-xs text-slate-500 font-medium mb-4 flex items-center gap-1.5">
                            ...
                            Sorteo: {new Date(getDrawDate(s)).toLocaleDateString('es-PE', { day: '2-digit', month: 'long', year: 'numeric' })}
                          </p>
                        )}
        ...
        {prizes.length > 0 && (
          <section id="premios" className="py-16 bg-[#F8FAFC] dark:bg-slate-950 transition-colors duration-300">
            <div className="container mx-auto px-4 max-w-6xl">
              <div className="text-center mb-10">
                <h2 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white uppercase">
                  Gana Uno de los
                  <span className="block text-emerald-600 dark:text-emerald-400 font-black mt-1">
                    {totalPrizesCount} Premios
                  </span>
                </h2>
                <div className="w-16 h-1.5 bg-amber-400 mx-auto rounded-full mt-6"></div>
                <p className="text-slate-500 dark:text-slate-400 mt-4 font-medium text-lg">Más de {totalPrizesCount} oportunidades de ganar en nuestro sorteo.</p>
              </div>

              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
                {prizes.map((prize) => (
                  <div key={prize.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden hover:shadow-xl transition-all group hover:-translate-y-1 hover:border-emerald-200 dark:hover:border-emerald-500 cursor-pointer">
                    <div className="relative aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-slate-700">
                      ...
                      <div className="absolute top-2 right-2 bg-[#FF0000] text-white font-black text-sm md:text-base px-2.5 py-1 rounded-lg shadow-md border-2 border-white dark:border-slate-900 z-10">
                        x{prize.qty}
                      </div>
                    </div>
              ...
              <div className="text-center mt-10">
                <a href="/participar" className="bg-amber-400 hover:bg-amber-500 text-slate-900 font-bold px-8 py-3 rounded-xl transition-colors shadow-sm inline-flex items-center gap-2">
        ...
        {grandWinners.length > 0 && (
          <section id="ganadores" className="py-16 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 overflow-hidden transition-colors duration-300">
            <div className="container mx-auto px-4 max-w-6xl relative">
              <div className="text-center mb-12">
                <h2 className="text-3xl font-black text-slate-900 dark:text-white uppercase flex justify-center items-center gap-3">
                  <Trophy className="text-amber-500 w-8 h-8"/> Nuestros ganadores
                </h2>
                <p className="text-slate-500 dark:text-slate-400 mt-2">Los ganadores más afortunados de nuestras últimas ediciones.</p>
              </div>

              <div className="relative mb-10">
                {/* Gradientes laterales */}
                <div className="pointer-events-none absolute inset-y-0 left-0 w-16 bg-gradient-to-r from-white dark:from-slate-900 to-transparent z-[1]" />
                <div className="pointer-events-none absolute inset-y-0 right-0 w-16 bg-gradient-to-l from-white dark:from-slate-900 to-transparent z-[1]" />

                {/* Carrusel continuo (sin duplicar) */}
                <div
                  ref={winnersWrapRef}
                  onPointerDown={() => setWinnersPaused(true)}
                  onPointerUp={() => window.setTimeout(() => setWinnersPaused(false), 600)}
                  onPointerCancel={() => setWinnersPaused(false)}
                  className="overflow-hidden px-2 md:px-3 pb-4"
                >
                  <div
                    ref={winnersTrackRef}
                    className={`flex gap-4 md:gap-6 will-change-transform ${singleWinner ? 'justify-center' : ''}`}
                  >
                    {grandWinners.map((winner) => (
                      <div key={winner.id} className={`w-[200px] md:w-[260px] shrink-0 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden group relative hover:border-emerald-200 dark:hover:border-emerald-500 transition-colors ${singleWinner ? 'winner-float' : ''}`}>
                      <div className="bg-slate-100 dark:bg-slate-700 aspect-square flex items-center justify-center relative overflow-hidden">
                        {winner.imagen ? (
                      ...
                      <div className="p-4 text-center">
                        <p className="font-bold text-slate-800 dark:text-slate-200 text-sm mb-1">{winner.name}</p>
                        <p className="text-emerald-600 dark:text-emerald-400 font-black text-sm mb-3">{winner.prize}</p>
                        <a
                          ...
                          className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase hover:text-emerald-700 dark:hover:text-emerald-400 transition-colors flex items-center justify-center gap-1 mx-auto bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-full inline-flex"
                        >
