...
export default function Ejecucion({ sorteosData = [] }) {
  ...
  const [execDraw, setExecDraw]         = useState('');    // nombre del sorteo
  ...
  const selectedSorteoData = React.useMemo(
    () => sorteosData.find(s => s.nombre === execDraw),
  ...
  const validTicketsCount = selectedSorteoData
    ? (selectedSorteoData.valid_tickets_count - drawnHistory.length)
    : 0;
      ...
      <div className="flex flex-col gap-6">
        ...
        <div
          ...
          className={`transition-all duration-500 flex flex-col items-center justify-center relative w-full ${
            execDraw ? 'bg-[#0f1b29]' : 'bg-slate-50'
          } ${
            fullscreen 
              ? 'fixed inset-0 z-50 rounded-none p-4 md:p-10 w-screen h-screen overflow-y-auto' 
              : 'rounded-[2rem] p-8 shadow-sm border min-h-[500px] ' + (execDraw ? 'border-[#1e2f45]' : 'border-slate-100')
          }`}
        >
          ...
          <div className="w-full max-w-6xl flex flex-col lg:flex-row gap-8 lg:gap-10 items-stretch justify-center z-10 flex-col-reverse lg:flex-row">
            ...
            <div className={`lg:w-[45%] flex flex-col justify-center space-y-6 w-full p-8 rounded-3xl transition-colors ${
              ...
              {/* Selector de Premio */}
              <div>
                <label className={`block text-[11px] font-black uppercase tracking-widest mb-3 ${execDraw ? 'text-amber-500' : 'text-slate-500'}`}>
              ...
              {!drawnTicket ? (
                <button
                  ...
                  disabled={!execDraw || !execPrizeId || isSpinning}
                  className={`w-full py-4 mt-2 rounded-xl font-black text-base flex items-center justify-center gap-3 transition-colors shadow-md ${
                    (!execDraw || !execPrizeId)
                      ? (execDraw ? 'bg-[#21354d] text-slate-500 cursor-not-allowed' : 'bg-slate-200 text-slate-400 cursor-not-allowed')
                ...
                <div className={`w-full p-6 rounded-xl border shadow-md text-center animate-fade-in-up mt-2 ${
                  ...
                  <p className={`text-2xl font-black mb-1 leading-tight ${execDraw ? 'text-white' : 'text-slate-800'}`}>{drawnTicket.user}</p>
                  <p className={`text-xs font-mono mb-5 ${execDraw ? 'text-slate-400' : 'text-slate-500'}`}>Ticket #{drawnTicket.number}</p>
                ...
                <div className="flex gap-4 md:gap-6 justify-center w-full">
                  {currentSpinNumber.split('').map((digit, index) => (
                    <div key={index} className="flex-1 max-w-[90px] lg:max-w-[110px] h-28 md:h-36 lg:h-44 bg-white rounded-[1.25rem] flex items-center justify-center shadow-inner relative overflow-hidden shrink-0">
                      ...
                      <div className={`absolute top-1/2 left-0 w-3 h-4 rounded-r-full -translate-y-1/2 ${execDraw ? 'bg-[#0b141e]' : 'bg-[#606775]'}`} />
                      <div className={`absolute top-1/2 right-0 w-3 h-4 rounded-l-full -translate-y-1/2 ${execDraw ? 'bg-[#0b141e]' : 'bg-[#606775]'}`} />
