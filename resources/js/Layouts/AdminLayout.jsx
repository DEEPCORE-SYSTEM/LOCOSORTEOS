    ...
    <div className="min-h-screen bg-[#F4F6F9] flex flex-col md:flex-row font-sans text-slate-900">
      ...
      <aside className="w-full md:w-64 bg-[#064E3B] text-slate-300 flex flex-col shrink-0 md:h-screen md:sticky md:top-0">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-x-auto md:overflow-y-auto flex md:flex-col gap-2 md:gap-0 [&::-webkit-scrollbar]:hidden">
          ...
          <Link href="/admin/ejecucion" className={`flex items-center gap-3 shrink-0 md:w-full text-left px-4 py-3 rounded-xl font-bold transition-colors text-amber-400 ${currentView === 'admin-ejecucion' ? 'bg-emerald-600 text-white shadow-md' : 'hover:bg-emerald-800 hover:text-white'}`}>
          ...
          <Link href="/admin/tickets" className={`flex items-center justify-between shrink-0 md:w-full text-left px-4 py-3 rounded-xl font-bold transition-colors ${['admin-tickets', 'admin-lista-tickets', 'admin-talonario'].includes(currentView) ? 'bg-emerald-600 text-white shadow-md' : 'hover:bg-emerald-800 hover:text-white'}`}>
            ...
            {pendingTicketsCount > 0 && <span className="bg-red-500 text-white text-[10px] px-2 py-0.5 rounded-full">{pendingTicketsCount}</span>}
          </Link>
          ...
          <Link href="/admin/difusion" className={`flex items-center gap-3 shrink-0 md:w-full text-left px-4 py-3 rounded-xl font-bold transition-colors ${currentView === 'admin-content' ? 'bg-emerald-600 text-white shadow-md' : 'hover:bg-emerald-800 hover:text-white'}`}>
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-4">
            <button className="bg-white p-2 rounded-full shadow-sm text-slate-500 hover:text-emerald-600 relative">
              ...
              {pendingTicketsCount > 0 && <span className="absolute top-0 right-0 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white"></span>}
            </button>
            ...
            <Link href="/" className="md:hidden p-2 text-slate-500 hover:text-red-600">
